...
class TextDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        target_tensor = torch.as_tensor(target).long()

        target = torch.nn.functional.one_hot(target_tensor, num_classes=80)
        ...
        return indices, torch.ravel(target).float(), target_tensor
